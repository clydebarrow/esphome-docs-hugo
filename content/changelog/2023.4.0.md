...
We've added a new [/components/voice_assistant]({{< ref "/components/voice_assistant" >}}) component that allows you to use ESPHome devices as an input
for [assist](https://www.home-assistant.io/voice_control/) in Home Assistant **2023.5 or later**.

With this also comes preliminary [microphone ]({{< relref "/components/microphone/index" >}}) support, which has been built in a way that multiple
...
team as a full time developer to help out on ESPHome. Keith created the [/components/sprinkler]({{< ref "/components/sprinkler" >}}),
[/components/climate/thermostat]({{< ref "/components/climate/thermostat" >}}), a bunch of the `ssd` display components and a few other components
...
With the introduction of the [/components/microphone/i2s_audio]({{< ref "/components/microphone/i2s_audio" >}}), the media player platform has some
required breaking changes to the YAML configuration. This involves moving the `i2s_lrclk_pin` and
`i2s_bclk_pin` to a new [/components/i2s_audio]({{< ref "/components/i2s_audio" >}}) component.
