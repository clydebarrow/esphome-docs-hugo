...
In this ESPHome release, we added new triggers to the [Voice Assistant ]({{< relref "/components/voice_assistant" >}}) component
...
```
As part of this change also, [safe_mode ]({{< relref "/components/safe_mode" >}}) has been pulled out into its own top level component.
...
made a new top level [1-wire ]({{< relref "/components/one_wire" >}}) component that will allow for this.
...
The [http_request ]({{< relref "/components/http_request" >}}) platform has been rewritten in this release to
add support when using ESP-IDF or when using the [rp2040 ]({{< relref "/components/rp2040" >}}) platform. This will
...
This is mostly an internal change, but could be used by [external components ]({{< relref "/components/external_components" >}})
or lambdas in YAML.
