...
This release brings with it lots of goodies, but most important is the [native API ]({{< relref "/components/api" >}}).
...
- The fastled effects have been renamed to `addressable_` for the new
  [Neopixelbus ]({{< relref "/components/light/neopixelbus" >}}) integration. See the validation error message
  for more info.
...
- Added [/components/substitutions]({{< ref "/components/substitutions" >}}) to reduce repeating across configs.
...
- Added a bunch of guides (and helpers) for creating custom components. Also new: `esphomeyaml.libraries`,
  `esphomeyaml.includes` and `esphomeyaml.platformio_options` ([/components/esphome]({{< ref "/components/esphome" >}}))
...
- You can now use Home Assistant to get time in ESPHome, so no more need for SNTP. See [/components/time/index]({{< ref "/components/time/index" >}}).
