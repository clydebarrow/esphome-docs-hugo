...
[ESP32 Camera Support ]({{< relref "/components/esp32_camera" >}}): This is huge. When integrating cameras
...
And thanks very much to :ghuser:`puuu` for the [SDS011 Particulate Matter integration ]({{< relref "/components/sensor/sds011" >}}) and
:ghuser:`mvturnho` for the newly added
[MPR121 Capacitive Touch sensor ]({{< relref "/components/binary_sensor/mpr121" >}}) support!
...
- The `timeout_meter` and `timeout_time` options of the ultrasonic sensor
  have been merged into `timeout`. ([/components/sensor/ultrasonic]({{< ref "/components/sensor/ultrasonic" >}}))
...
- Added `calibrate_linear` sensor filter for easier calibration of sensors
  ([sensor-filter-calibrate_linear]({{< ref "components/sensor/sensor-filter-multiply#sensor-filter-calibrate_linear" >}}))
...
- Improved validation. For small typos for config variables similar keys are automatically
  suggested.
- Added `esp8266_restore_from_flash` to [esphome section ]({{< relref "/components/esphome" >}}) which
...
- Rotary Encoders have a new `min_value` and `max_value` option. ([/components/sensor/rotary_encoder]({{< ref "/components/sensor/rotary_encoder" >}}))
