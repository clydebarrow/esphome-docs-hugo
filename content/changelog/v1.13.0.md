...
{{< img src="live-validation.png" alt="Image" caption="I also want to give special shout outs to these awesome people who have contributed a lot of" width="70.0%" class="center" >}}
...
- Covers now support position & tilt - you can use these with the new [endstop ]({{< relref "/components/cover/endstop" >}})
  and [time-based ]({{< relref "/components/cover/time_based" >}}) cover platforms.
...
- [Lights ]({{< relref "/components/light/index" >}}) now have some new actions: `light.dim_relative` and
...
- [RDM6300 ]({{< relref "/components/binary_sensor/rdm6300" >}}) now also has an `on_tag` trigger like the PN532.
- [Remote Receiver ]({{< relref "/components/remote_receiver" >}}) and [Remote Transmitter ]({{< relref "/components/remote_transmitter" >}})
...
- Added [``globals.set`` Action]({{< ref "components/globals#globals-set_action" >}}) action,
  [``script.is_running`` Condition]({{< ref "components/script#script-is_running_condition" >}}) condition,
  [``for`` Condition]({{< ref "automations/actions#for_condition" >}}) condition.
