...
{{< img src="captive_portal-ui.png" alt="Image" caption="With the new :doc:`captive portal </components/captive_portal>` component you can set up a fallback" width="60.0%" class="center" >}}
...
[captive portal example ]({{< relref "/components/captive_portal" >}}).
...
- The new [SIM800L integration ]({{< relref "/components/sim800l" >}}) lets you send SMS and
  with [DF-Player ]({{< relref "/components/dfplayer" >}}) you can play some music from your ESP 🎵

{{< img src="vscode-validation.png" alt="Image" caption="Thank you :ghuser:`glmnet` for your work!" width="60.0%" class="center" >}}

New Sensors
***********

As you can see in the grid at the top of this page, there's been no shortage of sensors either!
There are all kinds of new sensor types here: from [lightning detectors ⚡️ ]({{< relref "/components/sensor/as3935" >}}),
the laser distance measuring [vl53l0x ]({{< relref "/components/sensor/vl53l0x" >}}) to various environmental
sensors (thanks :ghuser:`TheEggi`, :ghuser:`valordk`!).

In list of supported power measuring chips has grown a lot with the 3-channel [ATM90E32 ]({{< relref "/components/sensor/atm90e32" >}}),
various PZEM-004Ts and the power monitoring chip in the [Shelly 2.5 ]({{< relref "/components/sensor/ade7953" >}})
...
[Tuya MCU protocol ]({{< relref "/components/light/tuya" >}}) (thanks :ghuser:`ssieb` and :ghuser:`brandond`!) has been added and major
...
{{< img src="github-1000-stars.png" alt="Image" caption="As always, thanks to all contributors, bug reporters and patrons! Without you this would not" width="75.0%" class="center" >}}
...
- The `xiaomi_` BLE platforms have been renamed to match their model numbers (:esphomepr:`755`).
   ...
   - `xiaomi_mijia` -> [xiaomi_lywsdcgq ]({{< relref "/components/sensor/xiaomi_lywsdcgq" >}})
   - `xiaomi_miflora` -> [xiaomi_hhccjcy01 ]({{< relref "/components/sensor/xiaomi_hhccjcy01" >}})
...
- The ESP32 LEDC `bit_depth` option has been removed. The best bit depth is now dynamically
  calculated (:esphomepr:`754`, [docs ]({{< relref "/components/output/ledc" >}})).

- For [pcf8574 ]({{< relref "/components/pcf8574" >}}), the `INPUT_PULLUP` option has been removed and replaced
  with `INPUT` (:esphomepr:`828`, [docs ]({{< relref "/components/pcf8574" >}})).
...
- All [native API ]({{< relref "/components/api" >}}) communication stubs are now automatically
...
- [Base Sensor Configuration]({{< ref "components/sensor/_index#config-sensor" >}}) have a new `force_update` option (:esphomepr:`783`).
- Add GPIO Switch [Interlocking]({{< ref "components/switch/gpio#switch-gpio-interlocking" >}}) (:esphomepr:`777`).
- Add a configurable priority for WiFi network selection (:esphomepr:`658`, [docs ]({{< relref "/components/wifi" >}})).
- Add [``script.wait`` Action]({{< ref "components/script#script-wait_action" >}}) action (:esphomepr:`778`).
...
- Add [``output.ledc.set_frequency`` Action]({{< ref "components/output/ledc#output-ledc-set_frequency_action" >}}) action (:esphomepr:`754`).
- Changing from an effect to a static color for addressable light now
  has per-LED transitions (:esphomepr:`750`).
- Add [``sensor.rotary_encoder.set_value`` Action]({{< ref "components/sensor/rotary_encoder#sensor-rotary_encoder-set_value_action" >}}) action (:esphomepr:`747`).
- You can now specify per-device descriptions in the dashboard interface
  (:esphomepr:`707` by :ghuser:`Anonym-tsk`, [docs ]({{< relref "/components/esphome" >}})).
- The [SPI bus ]({{< relref "/components/spi" >}}) now sends data over the hardware SPI bus if possible.
...
- Add support for calibrating MH-Z19 sensors (:esphomepr:`683` by :ghuser:`Anonym-tsk`, [docs ]({{< relref "/components/sensor/mhz19" >}})).
- Add BLE scan parameters for improving BLE detection percentage
  (:esphomepr:`769`, [docs ]({{< relref "/components/esp32_ble_tracker" >}})).
- Add configurable ignore bits to rc_switch_raw codes (:esphomepr:`650` by :ghuser:`mtl010957`,
  [docs ]({{< relref "/components/remote_receiver" >}})).
- New `restore` option has been added to [servos ]({{< relref "/components/servo" >}}) (:esphomepr:`829`).
- Add IR receiver support for coolix climate devices (:esphomepr:`645` by :ghuser:`glmnet`, [docs ]({{< relref "/components/climate/climate_ir" >}})).
- Add [``calibrate_polynomial``]({{< ref "components/sensor/sensor-filter-calibrate_polynomial#sensor-filter-calibrate_polynomial" >}}) sensor filter (:esphomepr:`642`).
- Allow setting the initial mode of HLW8012 sensors (:esphomepr:`611` by :ghuser:`brandond`, [docs ]({{< relref "/components/sensor/hlw8012" >}})).
- Add tilt actions to [template cover ]({{< relref "/components/cover/template" >}}) (:esphomepr:`577` by :ghuser:`mtl010957`).
- Add backlight handling for lcd_pcf8574 (:esphomepr:`573` by :ghuser:`adarazs`, [docs ]({{< relref "/components/display/lcd_display" >}})).
