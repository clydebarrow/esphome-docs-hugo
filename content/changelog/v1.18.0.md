...
Apart from a few new sensors, we now have built in support for [/components/external_components]({{< ref "/components/external_components" >}}), check it out. Thanks :ghuser:`OttoWinter` and :ghuser:`glmnet`

Also now, an ESPHome device is able to connect to other BLE devices and become a [/components/ble_client]({{< ref "/components/ble_client" >}}), reading sensors and controlling switches etc.
You can read basic sensor values using the [/components/sensor/ble_client]({{< ref "/components/sensor/ble_client" >}}) component or use the data displayed in the logs to build a new native component
