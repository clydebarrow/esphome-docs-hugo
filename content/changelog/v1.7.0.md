...
- While all [Automation]({{< ref "automations/_index#automation" >}}) were previously already performed on the ESP itself, they only
...
- All components can now be flagged `internal`. Doing so will prevent them from being represented in the front-end
  (like MQTT). Useful for [Automation]({{< ref "automations/_index#automation" >}}). See 😎
...
- There are two new triggers available now: [``on_boot``]({{< ref "components/esphome#esphome-on_boot" >}}) and
  [``on_shutdown``]({{< ref "components/esphome#esphome-on_shutdown" >}}) with which you can do some advanced cleanup/setup
