...
- While all [automations](automations/_index#automation) were previously already performed on the ESP itself, they only
...
- All components can now be flagged `internal`. Doing so will prevent them from being represented in the front-end
  (like MQTT). Useful for [on-device automations](automations/_index#automation). See 😎
...
- The `pull_mode` option of the {{< docref "/components/sensor/pulse_counter" "Pulse Counter" >}} has been removed, please
  use the [Pin Schema](guides/configuration-types#config-pin_schema) now instead. Additionally, the `internal_filter` option now only accepts
  [time](guides/configuration-types#config-time) units.
