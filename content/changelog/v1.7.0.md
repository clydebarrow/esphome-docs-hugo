...
- New [WiFi signal strength sensor ]({{< relref "/components/sensor/wifi_signal" >}}) by
...
- You can now create a BLE iBeacon with your ESP32s using the new
  [BLE Beacon component ]({{< relref "/components/esp32_ble_beacon" >}}).

- esphomelib has finally gained support for a [status LED ]({{< relref "/components/status_led" >}}). Now
...
- The [pulse counter sensor ]({{< relref "/components/sensor/pulse_counter" >}}) has been back-ported to the
  ESP8266, so now you can observe the number of pulses and frequency on any pin 🔢

- A new [duty cycle sensor ]({{< relref "/components/sensor/duty_cycle" >}}) has been added, allowing you to
...
- The new [remote receiver ]({{< relref "/components/remote_receiver" >}}) and
  [remote transmitter ]({{< relref "/components/remote_transmitter" >}}) components now allows you to use any 433MHz
...
- The Hass.io add-on now has a new `password` option with which you can secure your installation. See
  [/guides/getting_started_hassio]({{< ref "/guides/getting_started_hassio" >}}). 🔒

- Binary Sensors now have filters too. They can now be used to debounce any binary sensor and do some more
  complicated actions using lambdas. See [binary_sensor-filters]({{< ref "components/binary_sensor/_index#binary_sensor-filters" >}}). ⛹️‍
...
- The [/components/deep_sleep]({{< ref "/components/deep_sleep" >}}) now has a `wakeup_pin_mode` option for the ESP32. This option
...
- Added a new `build_path` option in the `esphomeyaml` section with which you can customize where
  esphomeyaml stores the platformio project files. See [/components/esphome]({{< ref "/components/esphome" >}}).

Breaking Changes
****************

- Fixed the [SHT3x-D ]({{< relref "/components/sensor/sht3xd" >}}) component and removed the `accuracy`
...
- The `receive_timeout` option has been removed from the [i2c component ]({{< relref "/components/i2c" >}}) as it
  turns out it didn't actually do anything.

- The `ir_transmitter` component has been renamed to [remote_transmitter ]({{< relref "/components/remote_transmitter" >}})
  as it now works with all kinds of protocols, not just infrared-based ones.

- The `pull_mode` option of the [Pulse Counter ]({{< relref "/components/sensor/pulse_counter" >}}) has been removed, please
