...
- **id** (**Required**, [config-id](#config-id)): The ID with which you will be able to reference the animation later
...
- **animation.next_frame**: Moves the animation to the next frame. This is equivalent to the `id(my_animation).next_frame();` lambda call.
  ...
  - **id** (**Required**, [config-id](#config-id)): The ID of the animation to animate.
