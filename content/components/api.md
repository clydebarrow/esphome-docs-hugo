...
{{< /note >}}
- **actions** (*Optional*, list): A list of user-defined actions. See [User-defined Actions](components/api#api-device-actions).
...
- **on_client_connected** (*Optional*, [Action](automations/actions#config-action)): An automation to perform when a client
  connects to the API. See [`on_client_connected` Trigger](components/api#api-on_client_connected_trigger).
- **on_client_disconnected** (*Optional*, [Action](automations/actions#config-action)): An automation to perform when a client
  disconnects from the API. See [`on_client_disconnected` Trigger](components/api#api-on_client_disconnected_trigger).
...
{{< note >}}
Be sure to [follow the instructions above](components/api#api-actions) to tell Home Assistant to allow
...
straight from ESPHome [Automations](automations/_index#automation).
...
{{< /note >}}
When using the native API with Home Assistant, you can perform Home Assistant actions straight from ESPHome [Automations](automations/_index#automation).
