...
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [I²C Bus]({{< ref "components/i2c#i2c" >}}) or [SPI Bus]({{< ref "components/spi#spi" >}}) and
outputs its data. Using the [``pn532`` Binary Sensor]({{< ref "components/binary_sensor/pn532#pn532-tag" >}}) you can then
...
([datasheet](https://cdn-shop.adafruit.com/datasheets/pn532ds.pdf), [Adafruit](https://www.adafruit.com/product/364))
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [SPI Bus]({{< ref "components/spi#spi" >}}) and
outputs its data. Using the [``pn532`` Binary Sensor]({{< ref "components/binary_sensor/pn532#pn532-tag" >}}) you can then
create individual binary sensors that track if an NFC/RFID tag is currently detected by the PN532.
...
- **cs_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin on the ESP that the chip select line
  is connected to.
- **update_interval** (*Optional*, [Time]({{< ref "guides/configuration-types#config-time" >}})): The duration of each scan on the PN532. This affects the
...
- **on_tag_removed** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform
  when a tag is removed. See [``on_tag_removed``]({{< ref "components/binary_sensor/pn532#pn532-on_tag_removed" >}}).
...
The `pn532` component allows you to use PN532 NFC/RFID controllers
([datasheet](https://cdn-shop.adafruit.com/datasheets/pn532ds.pdf), [Adafruit](https://www.adafruit.com/product/364))
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [I²C Bus]({{< ref "components/i2c#i2c" >}}) and
outputs its data. Using the [``pn532`` Binary Sensor]({{< ref "components/binary_sensor/pn532#pn532-tag" >}}) you can then
create individual binary sensors that track if an NFC/RFID tag is currently detected by the PN532.
...
- **on_tag** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform
  when a tag is read. See [pn532-on_tag]({{< ref "components/binary_sensor/pn532#pn532-on_tag" >}}).
- **i2c_id** (*Optional*, [I²C Bus]({{< ref "components/i2c#i2c" >}})): Manually specify the ID of the [ID]({{< ref "guides/configuration-types#config-id" >}}) if you want
...
See [NDEF reading]({{< ref "components/binary_sensor/pn532#pn532-ndef_reading" >}}) below for how to use the second `tag` parameter that is provided to this trigger.
...
App and send it to Home Assistant using the [``homeassistant.tag_scanned`` Action]({{< ref "components/api#api-homeassistant_tag_scanned_action" >}}). The `tag` variable is
