...
See [Setting Up Tags](components/binary_sensor/pn532#pn532-setting_up_tags) for information on how to setup individual binary sensors for this component.
...
- **cs_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): The pin on the ESP that the chip select line
  is connected to.
- **update_interval** (*Optional*, [Time](guides/configuration-types#config-time)): The duration of each scan on the PN532. This affects the
...
- **on_tag** (*Optional*, [Automation](automations/_index#automation)): An automation to perform
  when a tag is read. See [pn532-on_tag](components/binary_sensor/pn532#pn532-on_tag).
- **on_tag_removed** (*Optional*, [Automation](automations/_index#automation)): An automation to perform
  when a tag is removed. See [`on_tag_removed`](components/binary_sensor/pn532#pn532-on_tag_removed).
...
See [NDEF reading](components/binary_sensor/pn532#pn532-ndef_reading) below for how to use the second `tag` parameter that is provided to this trigger.
