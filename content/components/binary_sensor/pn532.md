...
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [I²C]({{< ref "components/i2c#i2c" >}}) or [SPI]({{< ref "components/spi#spi" >}}) and
outputs its data. Using the [PN532 binary sensors]({{< ref "components/binary_sensor/pn532#pn532-tag" >}}) you can then
create individual binary sensors that track if an NFC/RFID tag is currently detected by the PN532.

![See :ref:`pn532-setting_up_tags` for information on how to setup individual binary sensors for this component.](../images/pn532-full.jpg)
...
You will need to have the [I²C Bus]({{< ref "components/i2c#i2c" >}}) or the [SPI Bus]({{< ref "components/spi#spi" >}}) configured depending on your choice.

## Over SPI

The `pn532_spi` component allows you to use PN532 NFC/RFID controllers
([datasheet](https://cdn-shop.adafruit.com/datasheets/pn532ds.pdf), [Adafruit](https://www.adafruit.com/product/364))
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [SPI]({{< ref "components/spi#spi" >}}) and
outputs its data. Using the [PN532 binary sensors]({{< ref "components/binary_sensor/pn532#pn532-tag" >}}) you can then
create individual binary sensors that track if an NFC/RFID tag is currently detected by the PN532.
...
- **cs_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin on the ESP that the chip select line
  is connected to.
- **update_interval** (*Optional*, [config-time]({{< ref "guides/configuration-types#config-time" >}})): The duration of each scan on the PN532. This affects the
...
- **on_tag** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform
  when a tag is read. See [pn532-on_tag]({{< ref "components/binary_sensor/pn532#pn532-on_tag" >}}).
- **on_tag_removed** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform
  when a tag is removed. See [pn532-on_tag_removed]({{< ref "components/binary_sensor/pn532#pn532-on_tag_removed" >}}).
...
![Example for hooking up the PN532 via SPI. Notice the position of the two switches on the right.](../images/pn532-spi.jpg)
...
The `pn532` component allows you to use PN532 NFC/RFID controllers
([datasheet](https://cdn-shop.adafruit.com/datasheets/pn532ds.pdf), [Adafruit](https://www.adafruit.com/product/364))
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [I²C]({{< ref "components/i2c#i2c" >}}) and
outputs its data. Using the [PN532 binary sensors]({{< ref "components/binary_sensor/pn532#pn532-tag" >}}) you can then
create individual binary sensors that track if an NFC/RFID tag is currently detected by the PN532.
...
- **on_tag** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform
  when a tag is read. See [pn532-on_tag]({{< ref "components/binary_sensor/pn532#pn532-on_tag" >}}).
- **i2c_id** (*Optional*, [I²C Component]({{< ref "components/i2c#i2c" >}})): Manually specify the ID of the [config-id]({{< ref "guides/configuration-types#config-id" >}}) if you want
...
See [pn532-ndef_reading]({{< ref "components/binary_sensor/pn532#pn532-ndef_reading" >}}) below for how to use the second `tag` parameter that is provided to this trigger.
...
[template sensor ]({{< relref "/components/sensor/template" >}}).
...
App and send it to Home Assistant using the [api-homeassistant_tag_scanned_action]({{< ref "components/api#api-homeassistant_tag_scanned_action" >}}). The `tag` variable is
...
- [Edit this page on GitHub](https://github.com/clydebarrow/esphome-docs-hugo/blob/current/content/components/binary_sensor/pn532.md)
