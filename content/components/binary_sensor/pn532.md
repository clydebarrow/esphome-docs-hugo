...
See [pn532-setting_up_tags](#pn532-setting_up_tags) for information on how to setup individual binary sensors for this component.
...
- **cs_pin** (**Required**, [Pin Schema](#config-pin_schema)): The pin on the ESP that the chip select line
  is connected to.
- **update_interval** (*Optional*, [config-time](#config-time)): The duration of each scan on the PN532. This affects the
...
- **on_tag_removed** (*Optional*, [Automation](#automation)): An automation to perform
  when a tag is removed. See [pn532-on_tag_removed](#pn532-on_tag_removed).
...
- **on_tag** (*Optional*, [Automation](#automation)): An automation to perform
  when a tag is read. See [pn532-on_tag](#pn532-on_tag).
- **i2c_id** (*Optional*, [config-id](#config-id)): Manually specify the ID of the [I²C Component](#i2c) if you want
...
See [pn532-ndef_reading](#pn532-ndef_reading) below for how to use the second `tag` parameter that is provided to this trigger.
...
App and send it to Home Assistant using the [api-homeassistant_tag_scanned_action](#api-homeassistant_tag_scanned_action). The `tag` variable is
