...
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [SPI](components/spi#spi) or [I²C](components/i2c#i2c) and
outputs its data. Using the [PN532 binary sensors](components/binary_sensor/pn532#pn532-tag) you can then
...
You will need to have the [SPI Bus](components/spi#spi) or the [I²C Bus](components/i2c#i2c) configured depending on your choice.

## Over SPI

The `pn532_spi` component allows you to use PN532 NFC/RFID controllers
([datasheet](https://cdn-shop.adafruit.com/datasheets/pn532ds.pdf), [Adafruit](https://www.adafruit.com/product/364))
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [SPI](components/spi#spi) and
outputs its data. Using the [PN532 binary sensors](components/binary_sensor/pn532#pn532-tag) you can then
create individual binary sensors that track if an NFC/RFID tag is currently detected by the PN532.
...
- **on_tag_removed** (*Optional*, [Automation](automations/_index#automation)): An automation to perform
...
The `pn532` component allows you to use PN532 NFC/RFID controllers
([datasheet](https://cdn-shop.adafruit.com/datasheets/pn532ds.pdf), [Adafruit](https://www.adafruit.com/product/364))
with ESPHome. This component is a global hub that establishes the connection to the PN532 via [I²C](components/i2c#i2c) and
outputs its data. Using the [PN532 binary sensors](components/binary_sensor/pn532#pn532-tag) you can then
create individual binary sensors that track if an NFC/RFID tag is currently detected by the PN532.
...
- **on_tag** (*Optional*, [Automation](automations/_index#automation)): An automation to perform
  when a tag is read. See [pn532-on_tag]({{< ref "components/binary_sensor/pn532#pn532-on_tag" >}}).
- **i2c_id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID of the [I²C Component](components/i2c#i2c) if you want
