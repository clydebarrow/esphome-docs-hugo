...
- If you have a module as shown in the image above, it can only be used in SPI mode
  ([unless hacked](https://forum.arduino.cc/index.php?topic=442750.0)) and you must define a [SPI bus](components/spi#spi)
  in your configuration with both the **miso_pin** and **mosi_pin** set.

- If you have a RC522 which communicates via I²C (as on the M5 Stack), you need to have an [I²C](components/i2c#i2c) bus configured.

## Over SPI

The `rc522_spi` component allows you to use RC522 NFC/RFID controllers with ESPHome. This component is a global hub
that establishes the connection to the RC522 via [SPI](components/spi#spi). Using [RC522 binary sensors](components/binary_sensor/rc522#rc522-binary_sensor),
...
- **cs_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): The pin on the ESP that the chip select line
  is connected to.
- **reset_pin** (*Optional*, [Pin Schema](guides/configuration-types#config-pin_schema)): The pin connected to the RST line. Some tests
...
- **on_tag** (*Optional*, [Automation](automations/_index#automation)): An automation to perform when a tag is read. See
...
that establishes the connection to the RC522 via [I²C](components/i2c#i2c) (also available over SPI). Using the
[RC522 binary sensors](components/binary_sensor/rc522#rc522-binary_sensor) you can then create individual binary sensors that track if
