...
{{< img src="rc522-full.jpg" alt="Image" caption="The ``rc522`` component allows you to use RC522 NFC/RFID controllers" width="60.0%" class="center" >}}
...
You can also use the tag information directly within ESPHome automations/lambdas. See [Setting Up Tags]({{< ref "components/binary_sensor/rc522#rc522-setting_up_tags" >}}) for
...
- If you have a module as shown in the image above, it can only be used in SPI mode
  ([unless hacked](https://forum.arduino.cc/index.php?topic=442750.0)) and you must define a [SPI Bus]({{< ref "components/spi#spi" >}})
  in your configuration with both the **miso_pin** and **mosi_pin** set.

- If you have a RC522 which communicates via I²C (as on the M5 Stack), you need to have an [I²C Bus]({{< ref "components/i2c#i2c" >}}) bus configured.

## Over SPI

The `rc522_spi` component allows you to use RC522 NFC/RFID controllers with ESPHome. This component is a global hub
that establishes the connection to the RC522 via [``rc522`` Binary Sensor]({{< ref "components/binary_sensor/rc522#rc522-binary_sensor" >}}). Using [SPI Bus]({{< ref "components/spi#spi" >}}),
...
- **cs_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin on the ESP that the chip select line
  is connected to.
- **reset_pin** (*Optional*, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin connected to the RST line. Some tests
  shows the RC522 working okay without this.
- **update_interval** (*Optional*, [Time]({{< ref "guides/configuration-types#config-time" >}})): The duration of each scan on the RC522. This affects the
...
- **on_tag** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform when a tag is read. See
  [``on_tag`` Trigger]({{< ref "components/binary_sensor/rc522#rc522-on_tag" >}}).
- **on_tag_removed** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform after a tag is removed. See
  [``on_tag_removed`` Trigger]({{< ref "components/binary_sensor/rc522#rc522-on_tag_removed" >}}).

## Over I²C

The `rc522_i2c` component allows you to use RC522 NFC/RFID controllers with ESPHome. This component is a global hub
that establishes the connection to the RC522 via [I²C Bus]({{< ref "components/i2c#i2c" >}}) (also available over SPI). Using the
[``rc522`` Binary Sensor]({{< ref "components/binary_sensor/rc522#rc522-binary_sensor" >}}) you can then create individual binary sensors that track if
...
- [Edit this page on GitHub](https://github.com/clydebarrow/esphome-docs-hugo/blob/current/content/components/binary_sensor/rc522.md)
