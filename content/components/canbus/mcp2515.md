...
The MCP2515 communicates with ESPHome via the [SPI bus](components/spi#spi); to use it, you must have at least one
[SPI bus](components/spi#spi) with both the `mosi_pin` and `miso_pin` defined in your ESPHome configuration.
...
- **cs_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): Is used to signal to a SPI device when it should
