...
All [graphical display configuration](components/display/_index#display-configuration) options are available, plus the following.
...
To modify the SPI setting see [SPI bus](components/spi#spi) . The default **data_rate** is set to `40MHz` and the **spi_mode** mode is `MODE0` but some displays require `MODE3` (*).
...
YAML configuration; please see [color](components/display/_index#config-color) for more detail on this configuration section.
