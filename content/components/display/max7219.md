...
- **lambda** (*Optional*, [lambda](automations/templates#config-lambda)): The lambda to use for rendering the content on the MAX7219.
  See [Rendering Lambda](components/display/max7219#display-max7219_lambda) for more information.
...
The MAX7219 has a similar API to the fully fledged [Display Rendering Engine](components/display/_index#display-engine), but it's only a subset as the MAX7219
