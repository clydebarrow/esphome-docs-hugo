...
- **lambda** (*Optional*, [lambda]({{< ref "automations/templates#config-lambda" >}})): The lambda to use for rendering the content on the MAX7219.
  See [display-max7219_lambda]({{< ref "components/display/max7219#display-max7219_lambda" >}}) for more information.
...
The MAX7219 has a similar API to the fully fledged [display-engine]({{< ref "components/display/_index#display-engine" >}}), but it's only a subset as the MAX7219
...
not all characters can be represented. You can see a full list of characters [below]({{< ref "components/display/max7219#display-max7219_characters" >}}).
