...
- **time_id** (*Optional*, [config-id](#config-id)): ID of a {{< docref "/components/time" >}}. If set, the time will be synchronized with every connection.
- **disconnect_delay** (*Optional*, [config-time](#config-time)): The amount of time the BLE connection is maintained before being disconnected again. Defaults to `5s`.
- **update_interval** (*Optional*, [config-time](#config-time)): The interval to transmit the display data. Defaults to `60s`.
- **validity_period** (*Optional*, [config-time](#config-time)): The time periode for which the pvvx device should display the information. Defaults to `5min`.
- **lambda** (*Optional*, [lambda](#config-lambda)): The lambda to use to define the information to be displayed.
  See [display-pvvx_mithermometer_lambda](#display-pvvx_mithermometer_lambda) for more information.
