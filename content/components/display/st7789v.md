...
displays with ESPHome. Note that this component utilizes the 4-Wire [SPI bus](components/spi#spi).
...
{{< warning >}}
This component has been made redundant since the ST7789V is now supported by the [ILI9XXX component](components/display/ili9xxx#ili9xxx). It is recommended
...
- **offset_width** (**Required**, int): When `model` is set to "Custom", use this to specify the display's horizontal
  offset in pixels. This option may not be specified when the `model` is not set to "Custom". Defaults depends `model`.
- **cs_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): The CS pin. Defaults depends `model`.
- **dc_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): The DC pin. Defaults depends `model`.
- **reset_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): The RESET pin. Defaults depends `model`.
...
- **backlight_pin** (*Optional*, [Pin Schema](guides/configuration-types#config-pin_schema)): The display's backlight pin. May be required
