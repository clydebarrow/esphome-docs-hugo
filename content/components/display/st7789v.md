...
displays with ESPHome. Note that this component utilizes the 4-Wire [SPI Bus]({{< ref "components/spi#spi" >}}).

![ST7789V TFT LCD on TTGO T-Display module](../images/st7789v-full.jpg)
...
{{< warning >}}
This component has been made redundant since the ST7789V is now supported by the [Models]({{< ref "components/display/ili9xxx#ili9xxx" >}}). It is recommended
...
```
{{< note >}}
For more information about the font options see: [Font Renderer Component]({{< ref "components/font#display-fonts" >}}).
...
- **offset_width** (**Required**, int): When `model` is set to "Custom", use this to specify the display's horizontal
  offset in pixels. This option may not be specified when the `model` is not set to "Custom". Defaults depends `model`.
- **cs_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The CS pin. Defaults depends `model`.
- **dc_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The DC pin. Defaults depends `model`.
- **reset_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The RESET pin. Defaults depends `model`.
...
- **backlight_pin** (*Optional*, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The display's backlight pin. May be required
...
- **power_supply** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})):[power supply ]({{< relref "/components/power_supply" >}})ply>` to connect to
...
- [index]({{< ref "index/" >}})
- [Power Supply Component ]({{< relref "/components/power_supply" >}})
- :apiref:`st7789v_base/st7789v_base.h`
- [Edit this page on GitHub](https://github.com/clydebarrow/esphome-docs-hugo/blob/current/content/components/display/st7789v.md)
