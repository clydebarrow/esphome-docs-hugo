...
- **lambda** (*Optional*, [lambda](#config-lambda)): The lambda to use for rendering the content on the TM1621.
  See [display-tm1621_lambda](#display-tm1621_lambda) for more information.
...
The TM1621 has a similar API to the fully fledged [display-engine](#display-engine), but it's only a subset as the TM1621
...
```
Please see [display-printf](#display-printf) for a quick introduction into the `printf` formatting rules.
