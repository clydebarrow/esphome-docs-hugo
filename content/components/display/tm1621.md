...
- **lambda** (*Optional*, [lambda]({{< ref "automations/templates#config-lambda" >}})): The lambda to use for rendering the content on the TM1621.
  See [display-tm1621_lambda]({{< ref "components/display/tm1621#display-tm1621_lambda" >}}) for more information.
...
The TM1621 has a similar API to the fully fledged [display-engine]({{< ref "components/display/_index#display-engine" >}}), but it's only a subset as the TM1621
...
```
Please see [display-printf]({{< ref "components/display/_index#display-printf" >}}) for a quick introduction into the `printf` formatting rules.
