...
- **lambda** (*Optional*, [lambda](#config-lambda)): The lambda to use for rendering the content on the TM1637.
  See [display-tm1637_lambda](#display-tm1637_lambda) for more information.
...
- **tm1637_id** (*Optional*, :[config-id](#config-id)): The id of the tm1637 that should be used to scan the keys in case you are using multiple devices.
...
The TM1637 has a similar API to the fully fledged [display-engine](#display-engine), but it's only a subset as the TM1637
