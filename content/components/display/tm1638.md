---
description: "TM1638 7 Segment Display Keypad & LED Module"
...
![TM1638 8-Segment Display Keypad & LED Module.](../images/tm1638-full.jpg)
...
- **stb_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin you have the STB line hooked up to.
- **clk_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin you have the CLK line hooked up to.
- **dio_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin you have the DIO line hooked up to.
...
- **lambda** (*Optional*, [Templates]({{< ref "automations/templates#config-lambda" >}})): The lambda to use for rendering the content on the TM1638.
  See [Rendering Lambda]({{< ref "components/display/tm1638#display-tm1638_lambda" >}}) for more information.
...
The TM1638 has a similar API to the fully fledged [Display Rendering Engine]({{< ref "components/display/_index#display-engine" >}}), but it's only a subset as the TM1638
...
not all characters can be represented. You can see a full list of characters [All 7-Segment Characters]({{< ref "components/display/max7219#display-max7219_characters" >}}).
...
```
## LEDs
The TM1638 7 Segment Display Keypad & LED Module has 8 LED lights across the top or middle of the display, depending on the board.  These LEDs can be exposed as [outputs ]({{< relref "/components/output/index" >}}) or [switches ]({{< relref "/components/switch/index" >}}) with the led property set between 0 and 7.  The board has the LEDs labelled 1-8, but the leds use zero based addressing.   The LED labelled "1" is LED 0 in the configuration.
...
- [Edit this page on GitHub](https://github.com/clydebarrow/esphome-docs-hugo/blob/current/content/components/display/tm1638.md)
