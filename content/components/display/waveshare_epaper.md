...
The communication ESPHome has chosen to use for this integration is 4-wire [SPI](components/spi#spi), as it's the most stable and high-speed. So you need to make sure your board is set to the 4-wire SPI mode and have an `spi:` section in your
configuration.
...
- **cs_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): The CS pin.
- **dc_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): The DC pin.
...
{{< /warning >}}
- **busy_pin** (*Optional*, [Pin Schema](guides/configuration-types#config-pin_schema)): The BUSY pin. Defaults to not connected.
- **reset_pin** (*Optional*, [Pin Schema](guides/configuration-types#config-pin_schema)): The RESET pin. Defaults to not connected.
...
- **full_update_every** (*Optional*, int): E-Paper displays have two modes of switching to the next image: A partial
  ...
  all other models.
- **reset_duration** (*Optional*, [Time](guides/configuration-types#config-time)): Duration for the display reset operation. Defaults to `200ms`.
...
- **update_interval** (*Optional*, [Time](guides/configuration-types#config-time)): The interval to re-draw the screen. Defaults to `1s`, use `never` to only manually update the screen via `component.update`.
