...
- **timeout** (*Optional*, [config-time]({{< ref "guides/configuration-types#config-time" >}})): Timeout for request. Defaults to `4.5s`.
...
- **watchdog_timeout** (*Optional*, [config-time]({{< ref "guides/configuration-types#config-time" >}})): Change the watchdog timeout during connection/data transfer.
...
- `body` as `std::string` which contains the response body when `capture_response`
  (see [http_request-get_action]({{< ref "components/http_request#http_request-get_action" >}})) is set to `true`.
