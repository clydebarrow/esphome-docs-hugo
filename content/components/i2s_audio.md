...
- **i2s_lrclk_pin** (**Required**, [config-pin](#config-pin)): The GPIO pin to use for the I²S `LRCLK` *(Left/Right Clock)* signal, also referred to as `WS` *(Word Select)* or `FS` *(Frame Sync)*.
- **i2s_bclk_pin** (*Optional*, [config-pin](#config-pin)): The GPIO pin to use for the I²S `BCLK` *(Bit Clock)* signal, also referred to as `SCK` *(Serial Clock)*.
- **i2s_mclk_pin** (*Optional*, [config-pin](#config-pin)): The GPIO pin to use for the I²S `MCLK` *(Master Clock)* signal.
- **id** (*Optional*, [config-id](#config-id)): Manually specify the ID for this I²S bus if you need multiple.
