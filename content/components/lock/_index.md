...
- **on_lock** (*Optional*, [Action](automations/actions#config-action)): An automation to perform
  when the lock is locked. See [`lock.on_lock` / `lock.on_unlock` Trigger](components/lock/_index#lock-on_lock_unlock_trigger).
- **on_unlock** (*Optional*, [Action](automations/actions#config-action)): An automation to perform
  when the lock is unlocked. See [`lock.on_lock` / `lock.on_unlock` Trigger](components/lock/_index#lock-on_lock_unlock_trigger)..
