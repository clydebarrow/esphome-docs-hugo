...
{{< note >}}
The "severity" of a log message represents the importance of the message, i.e. how critical it is. The severity levels are defined in the [log levels](components/logger#logger-log_levels) section.
...
-  **hardware_uart** (*Optional*, string): The Hardware UART to use for logging. The default varies depending on
   the specific processor/chip and framework you are using. See the [table below](components/logger#logger-default_hardware_interfaces).
...
In the `format` option, you can use `printf`-style formatting (see [Formatted Text](components/display/_index#display-printf)).
...
-  **format** (**Required**, string): The format for the message in [printf-style](components/display/_index#display-printf).
-  **args** (*Optional*, list of [lambda](automations/templates#config-lambda)): The optional arguments for the
   format message.
-  **level** (*Optional*, string): The [log level](components/logger#logger-log_levels) to print the message
...
```
{{< note >}}
Logging will not work in the `on_message` trigger. You can't use the [logger.log](components/logger#logger-log_action) action
