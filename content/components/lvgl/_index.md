...
To use LVGL with a [display](components/_index#display-hw) in ESPHome, you'll need an ESP32 or RP2040. PSRAM is not a strict requirement but it is generally recommended, especially for large color displays.
...
The following configuration variables apply to the main `lvgl` component, in order to establish the principal operating conditions. Some [styling options](components/lvgl/_index#lvgl-theme) can be set at this level too, but only for inheritance purposes.
...
- **gradients** (*Optional*, list): A list of gradient definitions to use in *bg_grad* styles. See [below](components/lvgl/_index#lvgl-gradients) for more details.
...
Colors can be specified anywhere in the LVGL configuration either by referencing a preconfigured [ESPHome color](components/display/_index#config-color) ID or by representing the color in the common hexadecimal notation. For example, `0xFF0000` would be red.
...
- **bg_color** (*Optional*, [color](components/lvgl/_index#lvgl-color)): Color for the background of the widget. Defaults to `0xFFFFFF` (white).
- **bg_grad** (*Optional*, [gradient](components/lvgl/_index#lvgl-gradients)): A gradient to apply to the background.
- **bg_grad_color** (*Optional*, [color](components/lvgl/_index#lvgl-color)): Color to make the background gradually fade to. Defaults to `0` (black).
...
- **opa** (*Optional*, [opacity](components/lvgl/_index#lvgl-opacity)): Opacity of the entire widget. Inherited from parent. Defaults to `COVER`.
- **bg_opa** (*Optional*, [opacity](components/lvgl/_index#lvgl-opacity)): Opacity of the widget background.
- **opa_layered** (*Optional*, [opacity](components/lvgl/_index#lvgl-opacity)): Opacity of the entire layer the widget is on. Inherited from parent. Defaults to `COVER`.
...
- **bg_image_opa** (*Optional*, [opacity](components/lvgl/_index#lvgl-opacity)): Opacity of the background image of the widget.
- **bg_image_recolor** (*Optional*, [color](components/lvgl/_index#lvgl-color)): Color to mix with every pixel of the background image of the widget.
- **bg_image_recolor_opa** (*Optional*, [opacity](components/lvgl/_index#lvgl-opacity)): Opacity of the recoloring of the background image of the widget.
- **border_width** (*Optional*, int16): Set the width of the border in pixels. Defaults to `0`.
- **border_color** (*Optional*, [color](components/lvgl/_index#lvgl-color)): Color to draw borders of the widget. Defaults to `0` (black).
- **border_opa** (*Optional*, [opacity](components/lvgl/_index#lvgl-opacity)): Opacity of the borders of the widget.  Defaults to `COVER`.
...
- **color_filter_opa** (*Optional*, [opacity](components/lvgl/_index#lvgl-opacity)): Opacity of the color filter. Currently color filters are applied only by the default LVGL theme, this option allows the effect of those to be disabled by setting to `TRANSP`.
...
- **outline_color** (*Optional*, [color](components/lvgl/_index#lvgl-color)): Color used to draw an outline around the widget. Defaults to `0` (black).
- **outline_opa** (*Optional*, [opacity](components/lvgl/_index#lvgl-opacity)): Opacity of the outline of the widget. Defaults to `COVER`.
...
- **shadow_color** (*Optional*, [color](components/lvgl/_index#lvgl-color)): Color used to create a drop shadow under the widget. Defaults to `0` (black).
...
- **stops** (**Required**, list): A list of at least 2 color stop points. Each stop point has the following options:
    - **color** (**Required**, [Color](components/lvgl/_index#lvgl-color)): The color of the stop point.
...
- **time** (*Optional*, [Time](guides/configuration-types#config-time)): Duration of the page change animation. Defaults to `50ms`.
...
- **timeout** (**Required**, [templatable](automations/templates#config-templatable), int): Amount of [time](guides/configuration-types#config-time) expected since the last touch event.
...
The `on_idle` [triggers](automations/_index#automation) are activated when inactivity time becomes longer than the specified `timeout`. You can configure any desired number of timeouts with different actions.

- **timeout** (**Required**, [templatable](automations/templates#config-templatable), int): [Time](guides/configuration-types#config-time) that has elapsed since the last touch event, after which the trigger will be invoked.
