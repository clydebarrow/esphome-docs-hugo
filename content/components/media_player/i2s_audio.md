...
- **i2s_dout_pin** (**Required**, [Pin](guides/configuration-types#config-pin)): The GPIO pin to use for the I²S `DOUT/SDOUT` *(Data Out)* signal, also referred to as `SD/SDATA` *(Serial Data)* or `DACDAT` *(Digital to Analog Converter Data)*.
- **mute_pin** (*Optional*, [Pin Schema](guides/configuration-types#config-pin_schema)): The GPIO pin to use to mute the media player.
- **mode** (*Optional*, string): The mode of the I²S bus. Can be `mono` or `stereo`. Defaults to `mono`.
- **i2s_audio_id** (*Optional*, [ID](guides/configuration-types#config-id)): The ID of the [I²S Audio](components/i2s_audio#i2s_audio) you wish to use for this media player.
