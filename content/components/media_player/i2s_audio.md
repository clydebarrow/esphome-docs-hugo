...
- **i2s_dout_pin** (**Required**, [config-pin](#config-pin)): The GPIO pin to use for the I²S `DOUT/SDOUT` *(Data Out)* signal, also referred to as `SD/SDATA` *(Serial Data)* or `DACDAT` *(Digital to Analog Converter Data)*.
...
- **i2s_audio_id** (*Optional*, [config-id](#config-id)): The ID of the [I²S Audio](#i2s_audio) you wish to use for this media player.
