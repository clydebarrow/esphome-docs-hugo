...
This platform only works on ESP32 based chips using the [esp32-espidf_framework](#esp32-espidf_framework).
...
- **announcement_pipeline** (**Required**, Pipeline Schema): Configuration settings for the announcement pipeline.
  ...
  - **speaker** (**Required**, [config-id](#config-id)): The {{< docref "/components/speaker/index" "speaker" >}} to output the audio.
...
- **files** (*Optional*, list): A list of media files to build into the firmware for on-device playback.
    - **id** (**Required**, [config-id](#config-id)): Unique ID for the file.
...
- **media_file** (**Required**, [config-id](#config-id)): The ID of the media file.
