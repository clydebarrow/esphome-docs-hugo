...
This platform only works on ESP32 based chips using the [esp32-espidf_framework]({{< ref "components/esp32#esp32-espidf_framework" >}}).
...
- **announcement_pipeline** (**Required**, Pipeline Schema): Configuration settings for the announcement pipeline.
  ...
  - **speaker** (**Required**, [config-id]({{< ref "guides/configuration-types#config-id" >}})):[speaker ]({{< relref "/components/speaker/index" >}})dex>` to output the audio.
...
- **files** (*Optional*, list): A list of media files to build into the firmware for on-device playback.
    - **id** (**Required**, [config-id]({{< ref "guides/configuration-types#config-id" >}})): Unique ID for the file.
...
- **media_file** (**Required**, [config-id]({{< ref "guides/configuration-types#config-id" >}})): The ID of the media file.
