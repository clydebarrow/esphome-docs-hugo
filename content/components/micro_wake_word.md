...
- **microphone** (**Required**, [Microphone Source Configuration](components/microphone/_index#config-microphone-source)): The {{< docref "/components/microphone/index" "microphone" >}} settings to use for audio input.
...
- **models** (**Required**, list): The models to use. Only the first model is enabled by default on the first boot. Each model's enabled state is then saved/restored to/from the flash.
  ...
  - **id** (*Optional*, [ID](guides/configuration-types#config-id)): The optional ID used for the model actions below.
