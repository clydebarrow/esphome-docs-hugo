...
- **placeholder** (*Optional*, [config-id](#config-id)): ID of an {{< docref "/components/image" "Image" >}} to display while the downloaded image is not yet ready.
...
- **id** (**Required**, [config-id](#config-id)): The image to update the URL for.
