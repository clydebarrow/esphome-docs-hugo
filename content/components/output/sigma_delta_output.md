...
to output a floating-point value on a binary output. Unlike with [/components/output/slow_pwm]({{< ref "/components/output/slow_pwm" >}}),
it is possible to update the output value with each update cycle, not just at the end of a longer period.

![Comparison between a *Slow PWM* with a period of 100s and a *sigma-delta output* with an update interval of 1s](../images/sigma-delta-example.png)
...
PWM period of 10 seconds, for reasonably frequent updates, with [/components/output/slow_pwm]({{< ref "/components/output/slow_pwm" >}})
...
This component can be used as a drop-in replacement for [/components/output/slow_pwm]({{< ref "/components/output/slow_pwm" >}}) by changing the `platform` to
...
to be off for a while to e.g. cool down, [/components/output/slow_pwm]({{< ref "/components/output/slow_pwm" >}})
