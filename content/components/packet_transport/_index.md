...
- **update_interval** (*Optional*, [Time](guides/configuration-types#config-time)): Interval between full broadcasts. Defaults to 15s.
...
- **ping_pong_recycle_time** (*Optional*, [Time](guides/configuration-types#config-time)): Controls how often the ping-pong key is regenerated. Requires `ping_pong_enable` to be set. Defaults to 10 minutes. Can be set only on the consumer side.
