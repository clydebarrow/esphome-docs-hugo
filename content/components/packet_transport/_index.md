...
transports are [/components/uart]({{< ref "/components/uart" >}}) and [/components/udp]({{< ref "/components/udp" >}}).
...
- **update_interval** (*Optional*, [config-time]({{< ref "guides/configuration-types#config-time" >}})): Interval between full broadcasts. Defaults to 15s.
- **sensors** (*Optional*, list): A list of sensor IDs to be broadcast. Each entry may be just the sensor id, or may set a different id to be broadcast.
  ...
  - **id** (**Required**, [config-id]({{< ref "guides/configuration-types#config-id" >}})): The id of the sensor to be used
...
- **binary_sensors** (*Optional*, list): A list of binary sensor IDs to be broadcast.
  ...
  - **id** (**Required**, [config-id]({{< ref "guides/configuration-types#config-id" >}})): The id of the binary sensor to be used
...
- **ping_pong_recycle_time** (*Optional*, [config-time]({{< ref "guides/configuration-types#config-time" >}})): Controls how often the ping-pong key is regenerated. Requires `ping_pong_enable` to be set. Defaults to 10 minutes. Can be set only on the consumer side.
...
- [Edit this page on GitHub](https://github.com/clydebarrow/esphome-docs-hugo/blob/current/content/components/packet_transport/index.md)
