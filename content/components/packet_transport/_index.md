...
- **update_interval** (*Optional*, [config-time](#config-time)): Interval between full broadcasts. Defaults to 15s.
- **sensors** (*Optional*, list): A list of sensor IDs to be broadcast. Each entry may be just the sensor id, or may set a different id to be broadcast.
  ...
  - **id** (**Required**, [config-id](#config-id)): The id of the sensor to be used
...
- **binary_sensors** (*Optional*, list): A list of binary sensor IDs to be broadcast.
  ...
  - **id** (**Required**, [config-id](#config-id)): The id of the binary sensor to be used
...
- **ping_pong_recycle_time** (*Optional*, [config-time](#config-time)): Controls how often the ping-pong key is regenerated. Requires `ping_pong_enable` to be set. Defaults to 10 minutes. Can be set only on the consumer side.
