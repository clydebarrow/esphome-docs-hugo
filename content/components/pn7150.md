...
independently enabled and disabled by using the corresponding [pn7150-actions](#pn7150-actions) (see below).
...
- **emulation_message** (*Optional*, string): When scanned by another NFC card/tag reader (such as a smartphone), this
  ...
  tag reader/writer.
- **tag_ttl** (*Optional*, [config-time](#config-time)): The duration that must elapse after the PN7150 is no longer able to
...
See [pn7150-ndef_reading](#pn7150-ndef_reading) below for how to use the second `tag` parameter that is provided to this trigger.
...
using [api-homeassistant_tag_scanned_action](#api-homeassistant_tag_scanned_action).
...
ID contained within its NDEF message to Home Assistant using the [api-homeassistant_tag_scanned_action](#api-homeassistant_tag_scanned_action).
