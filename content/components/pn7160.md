...
establishes a connection to the PN7160 via [I²C Bus]({{< ref "components/i2c#i2c" >}}) or [SPI Bus]({{< ref "components/spi#spi" >}}).

{{< img src="pn716x-full.jpg" alt="Image" caption="Within ESPHome, the PN7160 can be configured to use either the SPI **or** I²C protocol for data communication." width="70.0%" class="center" >}}
...
[I²C Bus]({{< ref "components/i2c#i2c" >}}) or the [SPI Bus]({{< ref "components/spi#spi" >}}).
...
independently enabled and disabled by using the corresponding [Actions]({{< ref "components/pn7160#pn7160-actions" >}}) (see below).

In addition, the [binary_sensor/nfc]({{< ref "binary_sensor/nfc" >}}) platform may be used to quickly and easily identify tags presented to the reader.

{{< anchor "pn7160-spi" >}}

## Over SPI

The `pn7160_spi` component allows you to use [SPI Bus]({{< ref "components/spi#spi" >}}) PN7160 NFC controllers with with ESPHome.
...
- **cs_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin connected to the PN7160's `NSS` (chip
  select) line.
- **dwl_req_pin** (*Optional*, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin connected to the PN7160's
  `DWL_REQ` line. Used to invoke the PN7160's firmware update mode; may be used in a future release.
- **irq_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin connected to the PN7160's `IRQ` line.
- **ven_pin** (**Required**, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin connected to the PN7160's `VEN` line.
- **wkup_req_pin** (*Optional*, [Pin Schema]({{< ref "guides/configuration-types#config-pin_schema" >}})): The pin connected to the PN7160's
...
- **emulation_message** (*Optional*, string): When scanned by another NFC card/tag reader (such as a smartphone), this
  ...
  tag reader/writer.
- **tag_ttl** (*Optional*, [Time]({{< ref "guides/configuration-types#config-time" >}})): The duration that must elapse after the PN7160 is no longer able to
  "see" a tag before it is considered to have been removed from the reader.
- **on_tag** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform when a tag is first read. See
  [``on_tag`` Trigger]({{< ref "components/pn7160#pn7160-on_tag" >}}).
- **on_tag_removed** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform after a tag is removed. See
  [``on_tag_removed`` Trigger]({{< ref "components/pn7160#pn7160-on_tag_removed" >}}).
- **on_emulated_tag_scan** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform when the PN7160 is
  scanned by another tag reader (such as a smartphone). See [``on_emulated_tag_scan`` Trigger]({{< ref "components/pn7160#pn7160-on_emulated_tag_scan" >}}).
...
- **id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for this component.

{{< anchor "pn7160-i2c" >}}

## Over I²C

The `pn7160_i2c` component allows you to use [I²C Bus]({{< ref "components/i2c#i2c" >}}) PN7160 NFC controllers with ESPHome.
...
See [NDEF Reading]({{< ref "components/pn7160#pn7160-ndef_reading" >}}) below for how to use the second `tag` parameter that is provided to this trigger.
...
using [``homeassistant.tag_scanned`` Action]({{< ref "components/api#api-homeassistant_tag_scanned_action" >}}).
...
```
You could also send the value to Home Assistant via a [template sensor ]({{< relref "/components/sensor/template" >}}):
...
ID contained within its NDEF message to Home Assistant using the [``homeassistant.tag_scanned`` Action]({{< ref "components/api#api-homeassistant_tag_scanned_action" >}}).
...
[button ]({{< relref "/components/button/index" >}}) is a great mechanism to use to trigger these actions.
...
- [index]({{< ref "index/" >}})
- [binary_sensor/pn532]({{< ref "binary_sensor/pn532" >}})
- [binary_sensor/rc522]({{< ref "binary_sensor/rc522" >}})
- [binary_sensor/rdm6300]({{< ref "binary_sensor/rdm6300" >}})
- :apiref:`pn7160/pn7160.h`
- [Edit this page on GitHub](https://github.com/clydebarrow/esphome-docs-hugo/blob/current/content/components/pn7160.md)
