...
independently enabled and disabled by using the corresponding [pn7160-actions](#pn7160-actions) (see below).
...
- **emulation_message** (*Optional*, string): When scanned by another NFC card/tag reader (such as a smartphone), this
  ...
  tag reader/writer.
- **tag_ttl** (*Optional*, [config-time](#config-time)): The duration that must elapse after the PN7160 is no longer able to
...
See [pn7160-ndef_reading](#pn7160-ndef_reading) below for how to use the second `tag` parameter that is provided to this trigger.
