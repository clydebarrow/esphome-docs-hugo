...
independently enabled and disabled by using the corresponding [Actions](components/pn7160#pn7160-actions) (see below).
...
- **emulation_message** (*Optional*, string): When scanned by another NFC card/tag reader (such as a smartphone), this
  ...
  tag reader/writer.
- **tag_ttl** (*Optional*, [Time](guides/configuration-types#config-time)): The duration that must elapse after the PN7160 is no longer able to
...
- **on_tag** (*Optional*, [Automation](automations/_index#automation)): An automation to perform when a tag is first read. See
  [`on_tag` Trigger](components/pn7160#pn7160-on_tag).
- **on_tag_removed** (*Optional*, [Automation](automations/_index#automation)): An automation to perform after a tag is removed. See
  [`on_tag_removed` Trigger](components/pn7160#pn7160-on_tag_removed).
- **on_emulated_tag_scan** (*Optional*, [Automation](automations/_index#automation)): An automation to perform when the PN7160 is
  scanned by another tag reader (such as a smartphone). See [`on_emulated_tag_scan` Trigger](components/pn7160#pn7160-on_emulated_tag_scan).
...
See [NDEF Reading](components/pn7160#pn7160-ndef_reading) below for how to use the second `tag` parameter that is provided to this trigger.
