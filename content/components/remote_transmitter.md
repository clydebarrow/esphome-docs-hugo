...
- **pin** (**Required**, [Pin](guides/configuration-types#config-pin)): The pin to transmit the remote signal on.
...
- **on_transmit** (*Optional*, [Automation](automations/_index#automation)): An automation to perform before
  data is sent. Useful if the radio / IR hardware needs to change state or power on.
- **on_complete** (*Optional*, [Automation](automations/_index#automation)): An automation to perform after
...
- **repeat** (*Optional*): Defines the number of times the code is repeated when transmitted. By default, codes are
  ...
  - **times** ([templatable](automations/templates#config-templatable), int): The number of times to repeat the code.
  - **wait_time** ([templatable](automations/templates#config-templatable), [Time](guides/configuration-types#config-time)): The time to wait between repeats (in
...
- All other options from [Remote Transmitter Actions](components/remote_transmitter#remote_transmitter-transmit_action).
...
- **first** (**Required**, [templatable](automations/templates#config-templatable), uint32_t): The first 24-bit Coolix code to send;
  see dumper output for more info.
- **second** (*Optional*, [templatable](automations/templates#config-templatable), uint32_t): The second 24-bit Coolix code to send;
  see dumper output for more info.
- All other options from [Remote Transmitter Actions](components/remote_transmitter#remote_transmitter-transmit_action).
...
- **code** (**Required**, list, [templatable](automations/templates#config-templatable)): The 40-bit Midea code to send as a list of
  hex or integers.
- All other options from [Remote Transmitter Actions](components/remote_transmitter#remote_transmitter-transmit_action).
...
- **protocol** (*Optional*): The RC Switch protocol to use, see [RC Switch Protocol](components/remote_transmitter#remote_transmitter-rc_switch-protocol)
