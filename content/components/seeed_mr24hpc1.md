...
The [UART](components/uart#uart) is required to be set up in your configuration for this sensor to work, `parity` and `stop_bits` **must be** respectively `NONE` and `1`.
...
- **id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for this {{< docref "seeed_mr24hpc1/" >}} component if you need multiple components.
...
- **custom_presence_of_detection** (*Optional*, float): Valid only for [underlying open functions](components/seeed_mr24hpc1#seeed_mr24hpc1-open_function).
...
- **movement_signs** (*Optional*, int): A value calculated by a built-in algorithm to determine if someone is moving in the current environment.
  ...
  All options from [Sensor](components/sensor/_index#config-sensor).
- **custom_motion_distance** (*Optional*, float): Valid only for [underlying open functions](components/seeed_mr24hpc1#seeed_mr24hpc1-open_function).
  Distance in meters of detected moving target.
  All options from [Sensor](components/sensor/_index#config-sensor).
- **custom_spatial_static_value** (*Optional*, int): Valid only for [underlying open functions](components/seeed_mr24hpc1#seeed_mr24hpc1-open_function).
  ...
  All options from [Sensor](components/sensor/_index#config-sensor).
- **custom_spatial_motion_value** (*Optional*, int): Valid only for [underlying open functions](components/seeed_mr24hpc1#seeed_mr24hpc1-open_function).
  Motion amplitude values, different motion amplitudes cause different electromagnetic wave frequency changes.
  The output range for this value is 0-250.
  All options from [Sensor](components/sensor/_index#config-sensor).
- **custom_motion_speed** (*Optional*, float): Valid only for [underlying open functions](components/seeed_mr24hpc1#seeed_mr24hpc1-open_function).
...
- **sensitivity** (*Optional*, int): Valid only in [standard mode](components/seeed_mr24hpc1#seeed_mr24hpc1-standard_mode). Used to adjust the sensitivity of the radar.
  ...
  All options from [Number](components/number/_index#config-number).
...
- **custom_mode** (*Optional*, int): Settings and go to the Custom Mode option. Some of the function modules can only be set up in Custom Mode.
  ...
  All options from [Number](components/number/_index#config-number).
- **existence_threshold** (*Optional*, int): Valid only in [custom mode settings](components/seeed_mr24hpc1#seeed_mr24hpc1-custom_mode).
  This corresponds to [custom_spatial_static_value](components/seeed_mr24hpc1#seeed_mr24hpc1-spatial_static).
  ...
  All options from [Number](components/number/_index#config-number).
- **motion_threshold** (*Optional*, int): Valid only in [custom mode settings](components/seeed_mr24hpc1#seeed_mr24hpc1-custom_mode).
  This corresponds to [custom_spatial_motion_value](components/seeed_mr24hpc1#seeed_mr24hpc1-spatial_mtion).
  ...
  All options from [Number](components/number/_index#config-number).
- **motion_trigger** (*Optional*, int): Valid only in [custom mode settings](components/seeed_mr24hpc1#seeed_mr24hpc1-custom_mode).
  ...
  All options from [Number](components/number/_index#config-number).
- **motion_to_rest** (*Optional*, int): Valid only in [custom mode settings](components/seeed_mr24hpc1#seeed_mr24hpc1-custom_mode).
  ...
  All options from [Number](components/number/_index#config-number).
- **custom_unman_time** (*Optional*, int): Valid only in [custom mode settings](components/seeed_mr24hpc1#seeed_mr24hpc1-custom_mode).
...
- **restart** (*Optional*): Restart the device. All options from [Button](components/button/_index#config-button).
- **custom_set_end** (*Optional*): Valid only in [custom mode settings](components/seeed_mr24hpc1#seeed_mr24hpc1-custom_mode).
...
- **scene_mode** (*Optional*): Valid only in [standard mode](components/seeed_mr24hpc1#seeed_mr24hpc1-standard_mode). Used to select a preset scene in standard mode.
  ...
  All options from [Select](components/select/_index#config-select).
...
- **unman_time**: Valid only in [standard mode](components/seeed_mr24hpc1#seeed_mr24hpc1-standard_mode).
  Same as `custom_unman_time`, but this setting is only valid in standard mode.
  All options from [Select](components/select/_index#config-select).
- **existence_boundary**: Valid only in [custom mode settings](components/seeed_mr24hpc1#seeed_mr24hpc1-custom_mode).
  ...
  All options from [Select](components/select/_index#config-select).
- **motion_boundary**: Valid only in [custom mode settings](components/seeed_mr24hpc1#seeed_mr24hpc1-custom_mode).
