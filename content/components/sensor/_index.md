...
- **on_value** (*Optional*, [Automation](#automation)): An automation to perform
  when a new value is published. See [sensor-on_value](#sensor-on_value).
- **on_value_range** (*Optional*, [Automation](#automation)): An automation to perform
  when a published value transition from outside to a range to inside. See [sensor-on_value_range](#sensor-on_value_range).
- **on_raw_value** (*Optional*, [Automation](#automation)): An automation to perform
  when a raw value is received that hasn't passed through any filters. See [sensor-on_raw_value](#sensor-on_raw_value).

MQTT Options:

- **expire_after** (*Optional*, [config-time](#config-time)): Manually set the time in which
...
digital potentiometer or a DAC, the sensor automation [sensor-on_value](#sensor-on_value) can be used.
This makes it possible, for example, to replace a physical NTC and thus make it `smart`.

An output component can accept values in the range of `0..1` (see [output-set_level_action](#output-set_level_action)),
