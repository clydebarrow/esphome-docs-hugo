...
Communication with the device is done via an [SPI bus](components/spi#spi), so you need to have an `spi:` entry in your configuration
...
- **cs_pin** (**Required**, [Pin Schema](guides/configuration-types#config-pin_schema)): The pin CS is connected to. For the 6 channel meter main board, this will always be 5 and 4. For the add-on boards a jumper can be selected for each CS pin, but default to 0 and 16.
...
- **reference_voltage** (*Optional*): Fine grained gain calibration of a known voltage that a voltage transformer is connected to. It is used to calculate the value for `gain_voltage`. To be used in conjunction with the `run_gain_calibration` button.  `enable_gain_calibration` must be `True`. Each phase below defaults to `min_value: 100`, `max_value: 200`, `mode: box`, `optimistic: True`, `step: 0.1`.
  ...
  - **phase_a** (*Optional*): Phase A reference voltage field. All options from [Number](components/number/_index#config-number)
...
- **reference_current** (*Optional*):  Fine grained gain calibration for a known current of the circuit a CT is measuring. It is used to calculate the value for `gain_ct`. To be used in conjunction with the `run_gain_calibration` button. `enable_gain_calibration` must be `True`. Each phase below defaults to `min_value: 1`, `max_value: 200`, `mode: box`, `optimistic: True`, `step: 0.1`.
  ...
  - **phase_a** (*Optional*): Phase A reference current field. All options from [Number](components/number/_index#config-number)
  - **phase_b** (*Optional*): Phase B reference current field. All options from [Number](components/number/_index#config-number)
  - **phase_c** (*Optional*): Phase C reference current field. All options from [Number](components/number/_index#config-number)
