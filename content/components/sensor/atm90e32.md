...
- **id** (*Optional*, [config-id](#config-id)): Required if using more than one `atm90e32` chip with buttons for calibration, reference voltage/current fields, and phase status fields
...
- **id** (*Optional*, [config-id](#config-id)): The ID of the `atm90e32` sensor defined above. Required if using more than one sensor.
