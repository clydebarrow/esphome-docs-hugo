---
description: "Belling BL0942 Energy Monitor"
...
- **power** (*Optional*): The (active) power value of the sensor in Watts. Note that some power meters will report this in negative values (probably wired backwards), so you may want to use a filter to multiply it by -1. All options from [Base Sensor Configuration]({{< ref "components/sensor/_index#config-sensor" >}}).
...
- **power_reference** (*Optional*, float): The calibration parameter for power readings. Defaults to `596.0` unless either `current_reference` or `voltage_reference` are explicitly set, in which case it is calculated. See [Calibration]({{< ref "components/sensor/bl0942#bl0942-calibration" >}}) for more details.
- **energy_reference** (*Optional*, float): The calibration parameter for cumulative energy readings. Defaults to `3304.61127328` unless any of `current_reference`, `voltage_reference` or `power_reference` are explicitly set, in which case it is calculated. See [Calibration]({{< ref "components/sensor/bl0942#bl0942-calibration" >}}) for more details.
...
- [Edit this page on GitHub](https://github.com/clydebarrow/esphome-docs-hugo/blob/current/content/components/sensor/bl0942.md)
