...
{{< warning >}}
The BMP581's deep standby mode is disabled when you enable an IIR filter, which increases power consumption. Consider using a mathematically equivalent ESPHome exponential moving average filter instead, especially if you configure a long update interval. See the [exponential moving average filter]({{< ref "components/sensor/_index#sensor-filter-exponential_moving_average" >}}) for implementation information.
