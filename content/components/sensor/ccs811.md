...
- **baseline** (*Optional*, int): The baseline value for the unit, for calibration
  purposes. See [Calibrating Baseline](components/sensor/ccs811#ccs811-calibrating) for more info.

- **temperature** (*Optional*, [ID](guides/configuration-types#config-id)): Give an external temperature sensor ID
  here. This can improve the sensor's internal calculations.
- **humidity** (*Optional*, [ID](guides/configuration-types#config-id)): Give an external humidity sensor ID
  here. This can improve the sensor's internal calculations.
