...
- **baseline** (*Optional*, int): The baseline value for the unit, for calibration
  purposes. See [ccs811-calibrating](#ccs811-calibrating) for more info.

- **temperature** (*Optional*, [config-id](#config-id)): Give an external temperature sensor ID
  here. This can improve the sensor's internal calculations.
- **humidity** (*Optional*, [config-id](#config-id)): Give an external humidity sensor ID
  here. This can improve the sensor's internal calculations.
