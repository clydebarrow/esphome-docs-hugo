...
- **pin_s0** to **pin_s3** (**Required**, [config-pin](#config-pin)): The I/O pins connected to the S0 to S3 channel selection pins
- **delay** (*Optional*, [config-time](#config-time)): A small delay duration needed for the chip to switch inputs, defaults to 2ms.

## Sensor

### Configuration Variables:

- **sensor** (**Required**, [config-id](#config-id)): The source sensor to measure voltage values from, e.g. {{< docref "adc" "ADC sensor" >}}.
- **cd74hc4067_id** (**Required**, [config-id](#config-id)): The id of the cd74hc4067 component to use for this sensor.
