...
Your device communicates with the CS5460A over SPI so you need to have an [SPI bus](components/spi#spi) in
...
- **current** (*Optional*): The sensor subcomponent that will report RMS current values in Amperes.
  All options from [Sensor](components/sensor/_index#config-sensor).  See note below about throttling.
...
shorter, consider [filtering](components/sensor/_index#sensor-filters) the measurements if you have a Home
