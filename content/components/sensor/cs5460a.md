...
- **current** (*Optional*): The sensor subcomponent that will report RMS current values in Amperes.
  All options from [Sensor]({{< ref "components/sensor/_index#config-sensor" >}}).  See note below about throttling.
...
shorter, consider [filtering]({{< ref "components/sensor/_index#sensor-filters" >}}) the measurements if you have a Home
