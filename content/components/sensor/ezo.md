...
- **on_led** (*Optional*, [All Actions]({{< ref "automations/actions#config-action" >}})): Triggered when the result of `get_led_state()` is ready. The LED
  state is provided as a boolean variable named `x`.
- **on_device_information** (*Optional*, [All Actions]({{< ref "automations/actions#config-action" >}})): Triggered when the result of `get_device_information()`
  is ready.  The result is provided as a `std::string` variable named `x`.
- **on_slope** (*Optional*, [All Actions]({{< ref "automations/actions#config-action" >}})): Triggered when the result of `get_slope()` is ready.  The result
  is provided as a `std::string` variable named `x`.
- **on_calibration** (*Optional*, [All Actions]({{< ref "automations/actions#config-action" >}})): Triggered when the result of `get_calibration()` is ready.
  The result is provided as a `std::string` variable named `x`.
- **on_t** (*Optional*, [All Actions]({{< ref "automations/actions#config-action" >}})): Triggered when the result of `get_t()` is ready.  The result is provided
  as a `std::string` variable named `x`.
- **on_custom** (*Optional*, [All Actions]({{< ref "automations/actions#config-action" >}})): Triggered when the result of `get_custom()` is ready.  The result is provided as a `std::string` variable named `x`.

{{< anchor "ezo_lambda_calls" >}}

## Lambda calls

From [Templates]({{< ref "automations/templates#config-lambda" >}}), you can interact with the sensor in various ways. For any `get` command a trigger will be called
