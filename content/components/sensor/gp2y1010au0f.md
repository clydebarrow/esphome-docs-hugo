...
To use this sensor, you need to setup a voltage sensor source ([ADS1115 ]({{< relref "ads1115/" >}}), but for example also [ADC sensor ]({{< relref "adc/" >}})) and pass it to the sensor with the `sensor` option, in order to measure the sensor's output voltage pulses; you also need a binary [Output Component]({{< ref "components/output/_index#output" >}}) to control the sensor's internal IR LED.
