...
- **id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for this {{< docref "ld2410/" >}} component if you need multiple components.
...
- **has_still_target** (*Optional*): If true a still target is detected.
  All options from [Binary Sensor](components/binary_sensor/_index#config-binary_sensor).
- **out_pin_presence_status** (*Optional*): When in [engineering mode](components/sensor/ld2410#ld2410-engineering-mode), indicates whether the OUT pin indicates presence or not, otherwise `false`.
  OUT pin indication depends on the [light function](components/sensor/ld2410#ld2410-light-function) configuration. Might need latest firmware to work.
  All options from [Binary Sensor](components/binary_sensor/_index#config-binary_sensor).
- **ld2410_id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for the {{< docref "ld2410/" >}} component if you are using multiple components.
...
- **light** (*Optional*, int): When in [engineering mode](components/sensor/ld2410#ld2410-engineering-mode), indicates the light sensitivity, otherwise `unknown`.
...
- **gX** (*Optional*): Energies for the Xth gate (X => 0 to 8).
    ...
    - **move_energy** (*Optional*, int): When in [engineering mode](components/sensor/ld2410#ld2410-engineering-mode), the move energy of the gate, otherwise `unknown`.
      Value between `0` and `100` inclusive.
      All options from [Sensor](components/sensor/_index#config-sensor).
    - **still_energy** (*Optional*, int): When in [engineering mode](components/sensor/ld2410#ld2410-engineering-mode), the still energy of the gate, otherwise `unknown`.
      Value between `0` and `100` inclusive.
      All options from [Sensor](components/sensor/_index#config-sensor).
...
- **light_threshold** (*Optional*, int): Sets the light threshold for the [light function](components/sensor/ld2410#ld2410-light-function).
...
- **baud_rate** (*Optional*): Control the serial port baud rate. Defaults to `256000`.
  Once changed, all sensors will stop working until a fresh install with an updated [UART Component](components/uart#uart) configuration.
  All options from [Select](components/select/_index#config-select).
- **light_function** (*Optional*): If set, will affect the OUT pin value, based on [light threshold](components/sensor/ld2410#ld2410-light-threshold). Can be `off`, `low` or `above`. Defaults to `off`.
  All options from [Select](components/select/_index#config-select).
...
This is an [Action](automations/actions#config-action) for setting the bluetooth password.
...
- **id** (**Required**, [ID]({{< ref "guides/configuration-types#config-id" >}})): The ID of the {{< docref "ld2410/" >}} component to set.
- **password** (**Required**, string, [templatable](automations/templates#config-templatable)):
...
In order to monitor the presence indicated by the component, with the [light function](components/sensor/ld2410#ld2410-light-function) taken
under account, you can set up a [GPIO Binary Sensor](components/binary_sensor/gpio#gpio-binary-sensor):
...
1. Enable [engineering mode](components/sensor/ld2410#ld2410-engineering-mode).
2. Monitor the `gX_move_energy` and `gX_still_energy` [sensors](components/sensor/ld2410#ld2410-sensors).
3. Change the [thresholds](components/sensor/ld2410#ld2410-number) and repeat step 2 until satisfaction.
4. Disable [engineering mode](components/sensor/ld2410#ld2410-engineering-mode).
