...
The [UART]({{< ref "components/uart#uart" >}}) is required to be set up in your configuration for this sensor to work, `parity` and
...
- **id** (*Optional*, [config-id]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for [ld2450]({{< ref "ld2450/" >}})2450` component.
- **uart_id** (*Optional*, [UART Component]({{< ref "components/uart#uart" >}})): Manually specify the ID of the [config-id]({{< ref "guides/configuration-types#config-id" >}}) to use.
...
- **ld2450_id** (*Optional*, [config-id]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for[ld2450]({{< ref "ld2450/" >}})2450` component.
...
- **target_N** (*Optional*): Details about the targets (N = 1 to 3). A maximum of 3 targets can be detected:
  `target_1`, `target_2`, `target_3`.
    ...
    - **x** (*Optional*, int): Distance in `millimeter (mm)` of the target from the `ld2450` sensor along the
      ...
      detect targets from -3000 to 3000 mm in `X` direction. All options from [Sensor]({{< ref "components/sensor/_index#config-sensor" >}}).
...
- **ld2450_id** (*Optional*, [config-id]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for[ld2450]({{< ref "ld2450/" >}})2450` component if you are
  using multiple components.
- **bluetooth** (*Optional*): Turn on/off the bluetooth adapter. Defaults to `true`. All options from
  [Switch]({{< ref "components/switch/_index#config-switch" >}}).
- **multi_target** (*Optional*): Turn on/off the Multi Target Tracking option. The initial state set based on the
  corresponding setting as read from LD2450 module at boot. All options from [Switch]({{< ref "components/switch/_index#config-switch" >}}).
...
- **presence_timeout** (**Required**, int): The duration, in seconds, for which the
  ...
  seconds. All options from [Number]({{< ref "components/number/_index#config-number" >}}).
- **zone_N** (*Optional*): Configuration for the zone (N = 1 to 3), defined as a rectangular area in front of and
  ...
  **millimeters (mm)**.
    ...
    - **x1** (**Required**, int): Start X coordinate of the zone along the X-axis. Range is from -3000 (far left) to 3000
      (far right). All options from [Number]({{< ref "components/number/_index#config-number" >}}).
    - **y1** (**Required**, int): Start Y coordinate of the zone along the Y-axis. Range is 0 (closest) to 6000
      (furthest). All options from [Number]({{< ref "components/number/_index#config-number" >}}).
...
- **restart** (*Optional*): Restart the `ld2450` device. All options from [Button]({{< ref "components/button/_index#config-button" >}}).
...
- **target_N** (*Optional*): Targets (N = 1 to 3). A maximum of 3 targets can be detected: `target_1`, `target_2`,
  `target_3`.
    ...
    - **direction** (*Optional*): Direction of the target relative to the `ld2450` sensor. Possible values are:
      `Stationary`, `Moving away`, `Approaching`, `NA`. All options from [Sensor]({{< ref "components/sensor/_index#config-sensor" >}}).
...
- **baud_rate** (*Optional*): Control the UART serial port baud rate. Defaults to `256000`. Once changed, all sensors
  will stop working until a fresh install with an updated [UART Component]({{< ref "components/uart#uart" >}}) configuration. All options from
  [Select]({{< ref "components/select/_index#config-select" >}}).
- **zone_type** (*Optional*): Control the zone detection modes. It can be set to `Disabled`, `Detection` or
  ...
  from [Select]({{< ref "components/select/_index#config-select" >}}).
