...
can use the [HLKRadarTool mobile app<ld2450-mobile-apps>]({{< ref "#HLKRadarTool mobile app<ld2450-mobile-apps>" >}}) to update the firmware.

{{< /note >}}
![HLK-LD2450 radar module](../images/ld2450.png)
*HLK-LD2450 radar module*


The [UART Bus]({{< ref "components/uart#uart" >}}) is required to be set up in your configuration for this sensor to work, `parity` and
...
- **id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for [ld2450]({{< ref "ld2450/" >}})2450` component.
- **uart_id** (*Optional*, [UART Bus]({{< ref "components/uart#uart" >}})): Manually specify the ID of the [ID]({{< ref "guides/configuration-types#config-id" >}}) to use.
...
- **ld2450_id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for[ld2450]({{< ref "ld2450/" >}})2450` component.
...
- **target_N** (*Optional*): Details about the targets (N = 1 to 3). A maximum of 3 targets can be detected:
  `target_1`, `target_2`, `target_3`.
    ...
    - **x** (*Optional*, int): Distance in `millimeter (mm)` of the target from the `ld2450` sensor along the
      ...
      detect targets from -3000 to 3000 mm in `X` direction. All options from [Base Sensor Configuration]({{< ref "components/sensor/_index#config-sensor" >}}).
...
- **zone_N** (*Optional*): Target count details in the defined [zones<ld2450-number>]({{< ref "#zones<ld2450-number>" >}}) (N = 1 to 3). A maximum of
...
The `ld2450` switch allows you to control your [ld2450]({{< ref "ld2450/" >}}) `Bluetooth` and `Multi/Single Target Tracking`.
...
- **ld2450_id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID for[ld2450]({{< ref "ld2450/" >}})2450` component if you are
  using multiple components.
- **bluetooth** (*Optional*): Turn on/off the bluetooth adapter. Defaults to `true`. All options from
  [Base Switch Configuration]({{< ref "components/switch/_index#config-switch" >}}).
- **multi_target** (*Optional*): Turn on/off the Multi Target Tracking option. The initial state set based on the
  corresponding setting as read from LD2450 module at boot. All options from [Base Switch Configuration]({{< ref "components/switch/_index#config-switch" >}}).
...
- **presence_timeout** (**Required**, int): The duration, in seconds, for which the
  [presence states<ld2450-binary-sensors>]({{< ref "#presence states<ld2450-binary-sensors>" >}}) will persist even after the detection is cleared. Default is `5`
  seconds. All options from [Base Number Configuration]({{< ref "components/number/_index#config-number" >}}).
- **zone_N** (*Optional*): Configuration for the zone (N = 1 to 3), defined as a rectangular area in front of and
  ...
  **millimeters (mm)**.
    ...
    - **x1** (**Required**, int): Start X coordinate of the zone along the X-axis. Range is from -3000 (far left) to 3000
      (far right). All options from [Base Number Configuration]({{< ref "components/number/_index#config-number" >}}).
    - **y1** (**Required**, int): Start Y coordinate of the zone along the Y-axis. Range is 0 (closest) to 6000
      (furthest). All options from [Base Number Configuration]({{< ref "components/number/_index#config-number" >}}).
...
The `ld2450` button allows you to perfrom `restart` and `factory reset` actions on your [ld2450]({{< ref "ld2450/" >}}).
...
- **restart** (*Optional*): Restart the `ld2450` device. All options from [Base Button Configuration]({{< ref "components/button/_index#config-button" >}}).

## Text Sensor

The `ld2450` text sensor allows you to get information about your [ld2450]({{< ref "ld2450/" >}}).
...
- **target_N** (*Optional*): Targets (N = 1 to 3). A maximum of 3 targets can be detected: `target_1`, `target_2`,
  `target_3`.
    ...
    - **direction** (*Optional*): Direction of the target relative to the `ld2450` sensor. Possible values are:
      `Stationary`, `Moving away`, `Approaching`, `NA`. All options from [Base Sensor Configuration]({{< ref "components/sensor/_index#config-sensor" >}}).

## Select

The `ld2450` select allows you to control `baud rate` and `zone type` of your [ld2450]({{< ref "ld2450/" >}}).
...
- **baud_rate** (*Optional*): Control the UART serial port baud rate. Defaults to `256000`. Once changed, all sensors
  will stop working until a fresh install with an updated [UART Bus]({{< ref "components/uart#uart" >}}) configuration. All options from
  [Base Select Configuration]({{< ref "components/select/_index#config-select" >}}).
- **zone_type** (*Optional*): Control the zone detection modes. It can be set to `Disabled`, `Detection` or
  ...
  from [Base Select Configuration]({{< ref "components/select/_index#config-select" >}}).

## Example configuration:

Here is the complete example configuration for [ld2450]({{< ref "ld2450/" >}}).
...
- [Edit this page on GitHub](https://github.com/clydebarrow/esphome-docs-hugo/blob/current/content/components/sensor/ld2450.md)
