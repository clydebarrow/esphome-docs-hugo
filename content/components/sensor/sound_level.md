...
The `sound_level` sensor platform allows you to measure a {{< docref "/components/microphone" "microphone" >}}'s average and peak sound pressure levels over a specified measurement duration. The sensors output in **relative** `dB`, where `0 dB` represents the loudest sound the microphone can measure.
...
- **microphone** (**Required**, [Microphone Source Configuration]({{< ref "components/microphone/_index#config-microphone-source" >}})): The :doc:`micr{{< docref "/components/microphone" "microphone" >}}to use for input. Multiple channels may be selected.
