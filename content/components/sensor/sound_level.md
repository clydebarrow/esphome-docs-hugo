...
The `sound_level` sensor platform allows you to measure a [microphone ]({{< relref "/components/microphone/index" >}})'s average and peak sound pressure levels over a specified measurement duration. The sensors output in **relative** `dB`, where `0 dB` represents the loudest sound the microphone can measure.
