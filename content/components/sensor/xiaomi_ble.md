...
New firmware requires a bindkey in order to decrypt the received data (see [obtaining_the_bindkey](#obtaining_the_bindkey)), and stopped broadcasting battery level.
...
1. Xiaomi stock firmware (requires a bindkey in order to decrypt the received data, see [obtaining_the_bindkey](#obtaining_the_bindkey))
...
Hygro thermometer, square body, e-ink display, encrypted, broadcasts temperature, humidity and battery status. Requires a bindkey in order to decrypt the received data (see [obtaining_the_bindkey](#obtaining_the_bindkey)).
...
Cleargrass (Qingping) alarm clock, segment LCD, encrypted, broadcasts temperature, humidity and battery status. Requires a bindkey in order to decrypt the received data (see [obtaining_the_bindkey](#obtaining_the_bindkey)).
...
Cleargrass (Qingping) Temp & RH Lite, round body, segment LCD, encrypted, broadcasts temperature, humidity and battery status. Requires a bindkey in order to decrypt the received data (see [obtaining_the_bindkey](#obtaining_the_bindkey)).
...
Xiaomi Yeelight night light, in-shape replacement of MJYD02YL with BLE, broadcasts light on/off status, motion detection, idle time since last motion event and battery status. Requires a bindkey in order to decrypt the received data (see [obtaining_the_bindkey](#obtaining_the_bindkey)). Implemented as a hybrid sensor, needs both `sensor` and `binary_sensor` in config.
...
Qingping motion & ambient light sensor. Broadcasts motion detection, idle time since last motion event, lux value and battery status. Requires a bindkey in order to decrypt the received data (see [obtaining_the_bindkey](#obtaining_the_bindkey)).
...
Requires a bindkey in order to decrypt the received data (see [obtaining_the_bindkey](#obtaining_the_bindkey)).
