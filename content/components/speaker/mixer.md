...
The `mixer` speaker platform allows you to mix audio sent to different source speakers into one output which is sent to another {{< docref "/components/speaker" "speaker component" >}}. Individual source speakers may be ducked (made quieter) with the :ref:`apply ducking action <mixer_speaker-apply_ducking>`.
...
- **output_speaker** (**Required**, [ID]({{< ref "guides/configuration-types#config-id" >}})):{{< docref "/components/speaker" "speaker" >}}dex>` to output the mixed audio.
...
- **duration** (*Optional*, [Time]({{< ref "guides/configuration-types#config-time" >}}), templatable): The length of time to transition between the current reduction level and the new reduction level. Defaults to `0s`.
