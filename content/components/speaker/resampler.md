...
- **output_speaker** (**Required**, [config-id](#config-id)): The {{< docref "/components/speaker/index" "speaker" >}} to output the resampled audio.
- **buffer_duration** (*Optional*, [config-time](#config-time)): The duration of the internal ring buffer. Larger values may reduce stuttering but use more memory. Defaults to `500ms`.
