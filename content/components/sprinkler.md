...
number of [switch](components/switch/_index#config-switch) components, each of which would typically be used to control
...
```
Please see the [Controller Examples](components/sprinkler#sprinkler-controller-examples) section below for extensive, detailed configuration
...
- **auto_advance_switch** (*Optional*, *string*): Required with more than one valve. The name for the
  ...
  [The Sprinkler Controller Queue](components/sprinkler#sprinkler-controller-sprinkler_controller_queue) section below for more detail). This switch will
  not appear in the front end if the controller is configured with only one valve.
- **manual_selection_delay** (*Optional*, [Time](guides/configuration-types#config-time)): The amount of time the controller should
...
- **queue_enable_switch** (*Optional*, *string*): The name for the sprinkler controller's queue enable
  ...
  See [The Sprinkler Controller Queue](components/sprinkler#sprinkler-controller-sprinkler_controller_queue) section below for more detail.
...
- **valve_open_delay** (*Optional*, [Time](guides/configuration-types#config-time)): The *minimum* delay in seconds that should be
...
- **valve_overlap** (*Optional*, [Time](guides/configuration-types#config-time)): The amount of time in seconds that the current valve
...
- **pump_start_pump_delay** (*Optional*, [Time](guides/configuration-types#config-time)): The delay in seconds from when a distribution
...
- **pump_start_valve_delay** (*Optional*, [Time](guides/configuration-types#config-time)): The delay in seconds from when a pump
...
- **pump_stop_pump_delay** (*Optional*, [Time](guides/configuration-types#config-time)): The delay in seconds from when a distribution
...
- **pump_stop_valve_delay** (*Optional*, [Time](guides/configuration-types#config-time)): The delay in seconds from when a pump is
...
- **pump_pulse_duration** (*Optional*, [Time](guides/configuration-types#config-time)): The *minimum* length of the pulse generated to
...
- **valve_pulse_duration** (*Optional*, [Time](guides/configuration-types#config-time)): The *minimum* length of the pulse generated to
...
- **multiplier_number** (*Optional*, *string*): The name of the {{< docref "/components/number/index" "number" >}} component that
  ...
  value. See [Using the Sprinkler Controller's Numbers](components/sprinkler#sprinkler-controller-sprinkler_controller_numbers) below for more detail.
- **repeat_number** (*Optional*, *string*): The name of the {{< docref "/components/number/index" "number" >}} component that
  ...
  value. May not be used with `repeat`. See [Using the Sprinkler Controller's Numbers](components/sprinkler#sprinkler-controller-sprinkler_controller_numbers) below for more
  detail.
...
- **next_prev_ignore_disabled** (*Optional*, boolean): Set to `true` to cause
  [`sprinkler.next_valve` action](components/sprinkler#sprinkler-controller-action_next_valve) and [`sprinkler.previous_valve` action](components/sprinkler#sprinkler-controller-action_previous_valve) to skip
...
- **valves** (**Required**, *list*): A list of valves the controller should use. Each valve consists of:
  ...
  - **pump_switch_id** (*Optional*, [Switch](components/switch/_index#config-switch)): This is the [switch](components/switch/_index#config-switch)
    ...
    see* [An Important Note about GPIO Switches and Control](components/sprinkler#sprinkler-controller-an_important_note_about_gpio_switches_and_control) *below for more detail.*
    May not be specified with *pump_off_switch_id* or *pump_on_switch_id*.
  - **pump_off_switch_id** (*Optional*, [Switch](components/switch/_index#config-switch)): This is the [switch](components/switch/_index#config-switch)
  ...
  - **pump_on_switch_id** (*Optional*, [Switch](components/switch/_index#config-switch)): This is the [switch](components/switch/_index#config-switch)
  ...
  - **run_duration_number** (*Optional*, *string*): The name of the {{< docref "/components/number/index" "number" >}} component
    ...
    May not be used with `run_duration`. See [Using the Sprinkler Controller's Numbers](components/sprinkler#sprinkler-controller-sprinkler_controller_numbers) below for more
    detail. **Pro tip:** Want times in minutes? Add `unit_of_measurement: min` to the number configuration. See
    [Using the Sprinkler Controller's Numbers](components/sprinkler#sprinkler-controller-sprinkler_controller_numbers) for more detail.
  - **run_duration** (*Optional*, [Time](guides/configuration-types#config-time)): Required when `run_duration_number` is not provided. The
  ...
  - **valve_switch_id** (**Required**, [Switch](components/switch/_index#config-switch)): This is the [switch](components/switch/_index#config-switch)
    ...
    to expose this switch to the front end; please see* [An Important Note about GPIO Switches and Control](components/sprinkler#sprinkler-controller-an_important_note_about_gpio_switches_and_control)
    *below for more detail.* May not be specified with *valve_off_switch_id* or *valve_on_switch_id*.
  - **valve_off_switch_id** (**Required**, [Switch](components/switch/_index#config-switch)): This is the [switch](components/switch/_index#config-switch)
  ...
  - **valve_on_switch_id** (**Required**, [Switch](components/switch/_index#config-switch)): This is the [switch](components/switch/_index#config-switch)
...
limit of 100 entries to limit memory use.* Please see [The Sprinkler Controller Queue](components/sprinkler#sprinkler-controller-sprinkler_controller_queue)
...
Removes all queued valves from the controller's queue. Please see [The Sprinkler Controller Queue](components/sprinkler#sprinkler-controller-sprinkler_controller_queue)
section below for more detail and examples.
...
- The `next_valve` and `previous_valve` actions may not appear to respond immediately if either
  ...
  [Pump and Distribution Valve Coordination](components/sprinkler#sprinkler-controller-pump_and_distribution_valve_coordination) section below are configured.
...
could easily be added by adding the `pump_switch_id` parameter and a [switch](components/switch/_index#config-switch).
...
ESPHome's {{< docref "/components/number" >}}, supporting all of its [configuration variables](components/number/_index#config-number) in
...
ESPHome [switch](components/switch/_index#config-switch) components. Their configuration may be extended in a manner similar to
...
to take advantage of other devices such as a moisture [sensor](components/sensor/_index#config-sensor) -- when the moisture level
...
- **...determine if the sprinkler controller is paused and, if so, which valve is paused?**
  ...
  [active_valve() example above](components/sprinkler#sprinkler-controller-sprinkler_controller_understanding_state_how_do_i).
