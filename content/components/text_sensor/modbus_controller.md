...
- **register_count** (*Optional*, int): The number of consecutive registers this read request should span or skip in a single command. Default is 1. See [modbus_register_count](#modbus_register_count) for more details.
...
- **custom_command** (*Optional*, list of bytes): raw bytes for modbus command. This allows using non-standard commands. If `custom_command` is used `address` and `register_type` can't be used.
  ...
  See [modbus_custom_command](#modbus_custom_command) how to use `custom_command`
