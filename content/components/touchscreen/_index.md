...
- **update_interval** (*Optional*, [config-time]({{< ref "guides/configuration-types#config-time" >}})): The touchscreen polling interval - used only if an interrupt pin is not in use. Defaults to `50ms`.
- **touch_timeout** (*Optional*, [config-time]({{< ref "guides/configuration-types#config-time" >}})): A timeout for touchscreens that do not report the end of touch. The default varies depending on the touchscreen.
- **calibration** (*Optional*): Some touchscreens require calibration on a per-device basis.
    ...
    - **x_min** (**Required**, int): The raw value corresponding to the left
      (or top if `swap_xy` is specified) edge of the touchscreen. See [touchscreen-calibration]({{< ref "components/touchscreen/_index#touchscreen-calibration" >}})
...
- **on_touch** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform
  when the touchscreen is touched. See [touchscreen-on_touch]({{< ref "components/touchscreen/_index#touchscreen-on_touch" >}}).
- **on_update** (*Optional*, [Automation]({{< ref "automations/_index#automation" >}})): An automation to perform
  when the touchscreen is touched. See [touchscreen-on_update]({{< ref "components/touchscreen/_index#touchscreen-on_update" >}}).
...
Both the [touchscreen-on_update]({{< ref "components/touchscreen/_index#touchscreen-on_update" >}}) and [touchscreen-on_touch]({{< ref "components/touchscreen/_index#touchscreen-on_touch" >}}) have an argument of the type :apistruct:`touchscreen::TouchPoint` in a
...
[display-engine]({{< ref "components/display/_index#display-engine" >}}) component has to have the [0,0] logical coordinate at the top left.
...
NOTE: This has changed (from ESPHome 2023.11.6.) To receive continuous updates from touch drags, use [touchscreen-on_update]({{< ref "components/touchscreen/_index#touchscreen-on_update" >}}).
...
This trigger provides one argument named `touches` of type :apiref:`touchscreen::TouchPoints_t` which has a list of
 [touchscreen-touchpoint]({{< ref "components/touchscreen/_index#touchscreen-touchpoint" >}}).
...
- **page_id** (*Optional*, [config-id]({{< ref "guides/configuration-types#config-id" >}})): Only trigger this binary sensor if the display is showing this page.
  Cannot be used with `pages`.
- **pages** (*Optional*, list of [config-id]({{< ref "guides/configuration-types#config-id" >}})): Only trigger this binary sensor if the display is showing one of these pages.
