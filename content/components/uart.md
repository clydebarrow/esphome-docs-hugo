...
The UART component may be used as a platform for the [packet-transport]({{< ref "components/packet_transport/_index#packet-transport" >}}) component, enabling sensor data to be sent
...
- **tx_pin** (*Optional*, [config-pin]({{< ref "guides/configuration-types#config-pin" >}})): The pin to send data to from the ESP's perspective. Use the full pin schema and set `inverted: true` to invert logic levels. Not supported by host platform.
- **rx_pin** (*Optional*, [config-pin]({{< ref "guides/configuration-types#config-pin" >}})): The pin to receive data on from the ESP's perspective. Use the full pin schema and set `inverted: true` to invert logic levels. Not supported by host platform.
...
- **after** (*Optional*, mapping): The debugger accumulates bytes of communication. This option defines when
  ...
  - **timeout** (*Optional*, [config-time]({{< ref "guides/configuration-types#config-time" >}})): Trigger after no communication has been seen during the
