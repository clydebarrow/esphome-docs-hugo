...
The [UART Bus]({{< ref "components/uart#uart" >}}) must be configured with a baud rate of 19200, 8 data bits, no parity, 1 stop bit.
...
- **uart_id** (*Optional*, [UART Bus]({{< ref "components/uart#uart" >}})): Manually specify the ID of the [ID]({{< ref "guides/configuration-types#config-id" >}}) if you want to use multiple UART buses.
- **time_id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Specify the ID of[Time Component ]({{< relref "time/index" >}})dex>` to use as the time source if you want ESPHome to automatically synchronize the date and time of the thermostats.
...
- All options from [Base Climate Configuration]({{< ref "components/climate/_index#config-climate" >}}).
