...
- **uart_id** (*Optional*, [ID]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID of the UART hub used to connect to the device.
...
All sensors are *Optional* and support all other options from [Base Sensor Configuration]({{< ref "components/sensor/_index#config-sensor" >}}).
...
All binary sensors are *Optional* and support all other options from [Base Binary Sensor Configuration]({{< ref "components/binary_sensor/_index#config-binary_sensor" >}}).
...
- **sensors** (**Required**): A list of [Base Sensor Configuration]({{< ref "components/sensor/_index#config-sensor" >}}) definitions that include a `lambda` to do the decoding and return a `float` value.

- **lambda** (**Required**, [Templates]({{< ref "automations/templates#config-lambda" >}})): Code to parse a value from the incoming data packets and return it.
...
- **source** (**Required**): The address corresponding to `your device model` (see below).
- **command** (**Required**): The `command` corresponding to your device (see below).
- **binary_sensors** (**Required**): A list of [Base Binary Sensor Configuration]({{< ref "components/binary_sensor/_index#config-binary_sensor" >}}) definitions that include a `lambda` to do the decoding and return a `bool` value.
...
To extract the values with a [Templates]({{< ref "automations/templates#config-lambda" >}}), look in the packet structure by clicking the **Bytes** link in the table. Each value is placed at an `offset` within the packet.
