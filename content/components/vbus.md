...
- **uart_id** (*Optional*, [config-id]({{< ref "guides/configuration-types#config-id" >}})): Manually specify the ID of the UART hub used to connect to the device.
...
- **sensors** (**Required**): A list of [Sensor]({{< ref "components/sensor/_index#config-sensor" >}}) definitions that include a `lambda` to do the decoding and return a `float` value.
...
- **source** (**Required**): The address corresponding to `your device model` (see below).
- **command** (**Required**): The `command` corresponding to your device (see below).
- **binary_sensors** (**Required**): A list of [Binary Sensor]({{< ref "components/binary_sensor/_index#config-binary_sensor" >}}) definitions that include a `lambda` to do the decoding and return a `bool` value.
