...
through any browser and a simple [REST API](web-api/_index#api-rest). Please note that enabling this component
...
- **sorting_groups** (*Optional*, list): Available only on `version: 3`. A list of group ID's and names to group the entities. See [Webserver Entity Grouping](components/web_server#config-webserver-grouping).
  ...
  - **id** (**Required**, [ID](guides/configuration-types#config-id)): Manually specify the ID used for the group.
...
- [Event Source API](web-api/_index#api-event-source)
- [REST API](web-api/_index#api-rest)
