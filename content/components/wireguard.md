...
If you don't have it please read the section [wireguard-installation](#wireguard-installation).
...
- **netmask** (*Optional*, IPv4 address): The netmask for the configured address.
  ...
  See section [wireguard-static-routes](#wireguard-static-routes) if outgoing connections are
...
- **peer_persistent_keepalive** (*Optional*, [config-time](#config-time)): The amount of
...
- **reboot_timeout** (*Optional*, [config-time](#config-time)): The amount of time to wait
...
- **update_interval** (*Optional*, [config-time](#config-time)): How often to check
...
in the [esphome-on_boot](#esphome-on_boot) step.
